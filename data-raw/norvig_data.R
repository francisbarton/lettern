...
norvig_data <- rvest::read_html(norvig_url)


# Word length frequencies -------------------------------------------------

word_length_frequencies <- norvig_data |>
...
letter_frequencies_overall <- norvig_data |>
  rvest::html_element("body") |>
...
# "Now we show the letter frequencies by position within word.
# That is, the frequencies for just the first letter in each word, just the
# second letter, and so on.
# We also show frequencies for positions relative to the end of the word: "-1"
# means the last letter, "-2" means the second to last, and so on."

letter_frequencies_by_position <- norvig_data |>
  rvest::html_element("body") |>
...
# "Now we turn to sequences of letters: consecutive letters anywhere within a
# word. Below is a table of all 26 × 26 = 676 bigrams; in each cell the orange
# bar is proportional to the frequency, and if you hover you can see the exact
# counts and percentage. There are only seven bigrams that do not occur among
# the 2.8 trillion mentions: JQ, QG, QK, QY, QZ, WQ, and WZ."

bigram_frequencies <- norvig_data |>
  rvest::html_element("body") |>
